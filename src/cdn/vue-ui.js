...
var resize = function () {
    ...
    if(pc&&pc[0]){
		...
		if(pc[0].nodeName=="ION-CONTENT"){
			p=pc[0];
			e=p.children;
			for(i=0;i<e.length;i++){//v-dataview
				var event=new Event("parentResize2", {bubbles: true});
				event.height=h;
		...
		if(ww[0]){
			...
			for(kk=0;kk<ww[0].childNodes.length;kk++){
				if(ww[0].childNodes[kk].nodeName=='ION-HEADER')
					aft=1;
				else if(aft==1&&ww[0].childNodes[kk].nodeName!='ION-HEADER'&&ww[0].childNodes[kk].nodeName!='ION-FOOTER'){
					var t=ww[0].childNodes[kk];
					var hh=ww[0].offsetHeight-hr;
					t.style.height=hh+'px';
					t.style.overflowY='auto';
					var h1=t.children[0];
					var h2=t.children[1];
					var style = window.getComputedStyle(h1);
					hh=hh-h1.offsetHeight-(parseInt(style.paddingTop)+parseInt(style.paddingBottom));
					h2.style.height=hh+'px';
					h2.style.overflowY='auto';
					evt2=new Event("parentResize", {bubbles: true});
					evt2.height=hh;
					h2.children[0].children[0].style.height=hh+'px';
					h2.children[0].children[0].dispatchEvent(evt2);
					break;
				}
			}
