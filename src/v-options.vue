<template>
	...
	<option v-for="item in filterList" v-bind:value="getValueField(item)">{{item[displayField]}}
...
export default {
    ...
    data(){
        return {queue:[],data2: [],valueField_: [],lastLoad:null,loaded:null};
    },
    computed: {
        filterList() {
            var me=this;
...
//                    console.log('se encontro '+v+' en index='+j);
        ...
        async load(p, nou,clearQueue) {
            ...
            if(me.store){
                ...
                if(key){
                    me.data2=me.data2.concat(storedList.filter(item =>{
            ...
            if((me.url||me.src)&&!pa.disabled){
                ...
                await axios.get(me.url?me.url:me.src, {params: p})
                    .then((r)=>{
